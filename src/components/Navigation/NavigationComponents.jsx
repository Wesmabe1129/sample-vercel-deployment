...
const NavigationComponents = () => {
  return (
    <div className="fixed w-screen p-4 md:px-20  bottom-4 left-1/2 -translate-x-1/2 z-1000">
      
      {/* Inner flex container for nav items */}
      <div className="flex bg-black/10 backdrop-blur-sm p-2 sm:mx-8 md:mx-20 lg:mx-60 xl:mx-72 justify-between items-center gap-4 border-2 border-[#3B9797] rounded-[12px] shadow-lg z-50 bg-white/10 backdrop-blur-md">
        {/* Home */}
        <a href="#home" className="p-2 justify-center 2xl:justify-between sm:p-3 lg:w-[8rem] rounded-lg bg-teal-600 hover:bg-teal-700 transition flex items-center gap-2">
          <img src={homeImg} alt="Home" className="w-6 h-6 sm:w-8 sm:h-8" />
          <span className="hidden 2xl:inline text-white">Home</span>
        </a>

        {/* About */}
        <a href="#about" className="p-2 justify-center 2xl:justify-between sm:p-3 lg:w-[8rem] rounded-lg bg-teal-600 hover:bg-teal-700 transition flex items-center gap-2">
          <img src={aboutImg} alt="About" className="w-6 h-6 sm:w-8 sm:h-8" />
          <span className="hidden 2xl:inline text-white">About</span>
        </a>

        {/* Projects */}
        <a href="#projects" className="p-2 justify-center 2xl:justify-between sm:p-3 lg:w-[8rem] rounded-lg bg-teal-600 hover:bg-teal-700 transition flex items-center gap-2">
          <img src={projectsImg} alt="Projects" className="w-6 h-6 sm:w-8 sm:h-8" />
          <span className="hidden 2xl:inline text-white">Projects</span>
        </a>

        {/* Skills */}
        <a href="#skills" className="p-2 justify-center 2xl:justify-between sm:p-3 lg:w-[8rem] rounded-lg bg-teal-600 hover:bg-teal-700 transition flex items-center gap-2">
          <img src={skillsImg} alt="Skills" className="w-6 h-6 sm:w-8 sm:h-8" />
          <span className="hidden 2xl:inline text-white">Skills</span>
        </a>

        {/* Contacts */}
        <a href="#contacts" className="p-2 justify-center 2xl:justify-between sm:p-3 lg:w-[8rem] rounded-lg bg-teal-600 hover:bg-teal-700 transition flex items-center gap-2">
          <img src={contactsImg} alt="Contacts" className="w-6 h-6 sm:w-8 sm:h-8" />
          <span className="hidden 2xl:inline text-white">Contacts</span>
        </a>
      </div>
